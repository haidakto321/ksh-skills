...
The aisp orchestrator guides you through the full SDLC flow in a structured, human-gated pipeline. It judges task size and proposes a short flow for small changes, or runs the full flow for complex work. Human approval is required at the important gates - after spec, after plan, and after review - to keep stakeholders in the loop and prevent rework.
...
Watch for scope creep during code, hidden dependencies in the plan, and skipped gates. If the human goes silent at a gate, pause and ask again - silence does not mean approval. Flaky tests that spin the auto-fix loop should be escalated to /aisp-fix, not retried forever.

## Verification

Exit only when every mandatory gate was honored (explicit human approval after spec, after plan, after review) and the test step ended either green or escalated to /aisp-fix. If you skipped a gate, validation fails.
